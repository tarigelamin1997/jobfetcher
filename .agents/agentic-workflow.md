...
**Plus genuinely external eyes on every PR:** the **human**, and — *when it actually runs* — an automated reviewer (**CodeRabbit**, in the current runner). They complement, never replace, the Examiner.
>
> ⚠️ **Read the CodeRabbit check's description, never just its colour.** Its OSS free tier does not auto-review repositories under **10 stars** and reports that skip as a **green `success` check** — which is how it reviewed nothing on **34 consecutive PRs (#36–#69)** while every one showed a passing tick ([ERR-018](../docs/ledgers/errors.md)). **The threshold was cleared on 2026-09-05 (11 stars) and automatic review is back**, but the hazard it exposed is permanent: **three different outcomes all return a green `success`** — `Review completed`, `Review skipped: manual review required for this OSS repository`, and `Review rate limited` (a rolling account-level budget that can still refuse a PR outright, stars or no stars). The colour carries no information; only the `description` does. A `@coderabbitai review` comment **requests** a run when one is missing — best-effort, not a guarantee, because rate limiting can still refuse it; **check the resulting `description`**. `@coderabbitai rate limit` reports remaining capacity and does not itself consume any.
...
2. **Investigator** verifies it on live code/data (read-only) and writes a **durable dossier** — one case folder per bottleneck, [`docs/investigations/INV-NNN-<slug>/README.md`](../docs/investigations/), via [`/investigate`](../.claude/commands/investigate.md) — problem+evidence · root cause · blast radius · minimal change · files+reuse · validation gate [behavioral + a negative case] · out-of-scope · typed connections — and classifies severity in it. **It can kill the unit** (`status: killed`). The Surgeon then builds from the dossier, not an in-context brief; at close it fills the dossier's **Resolution — as-built** section so the folder stays a self-contained archive.
...
6. **Scribe close-out** (orchestrator): CHANGELOG `[Unreleased]` entry, ledger rows ([interface-contracts](../docs/ledgers/interface-contracts.md) Produces · [phase-index](../docs/ledgers/phase-index.md) · [backlog](../docs/ledgers/backlog.md)), an [ADR](../docs/adr/) if it's a real decision, and — if the unit came from a dossier — filling that dossier's **Resolution — as-built** section (what shipped · rung taken/divergence · key files · PR/ADR/CHANGELOG links · how to extend) and setting its `status: fixed`.
   ...
   **Human — nothing checks this, and pretending otherwise is how the debt started.**
   - **Read the external reviewer's *description*, not its colour.** `gh pr view <n> --json statusCheckRollup` and look at `description`. A **green `CodeRabbit`** check reading *"Review skipped: manual review required for this OSS repository"* **or** *"Review rate limited"* means **it reviewed nothing** — that is [ERR-018](../docs/ledgers/errors.md), and it held for 34 consecutive PRs while every one of them looked reviewed. **Deliberately NOT wired into CI:** making the build interrogate a third-party status would couple it to an external service's response shape to guard a gate that returns on its own at 10 stars — it fails the defensibility rubric. So this is a **suggestion, and is labelled one**; calling it a control would repeat the exact error the entry documents. No script can tell whether *this* unit added a component that belongs in the as-built picture, or changed what the system fundamentally *is*. **The judgement to make explicitly, in the PR body:** *did this unit add or change a surface, a resource, or a behaviour a first-time reader must know about? If yes — `README.md`, `CLAUDE.md`, `02-architecture.md`, `diagrams.md`.* Answer it in writing even when the answer is "no", because an unanswered question is what produced ERR-016; a written "no" is auditable, a skipped one is invisible.
...
- **Branch / PR / protected `main`:** one branch → one PR → required checks (`lint-and-test` · `terraform-validate` · `secret-scan`) + a `@coderabbitai review` comment (see the ERR-018 warning above — its check is green whether or not it ran) → merge (squash) → delete the branch → prune the worktree ([ADR-0013](../docs/adr/0013-enforcement-gate-trio-branch-pr.md)).
