...
>
> ⚠️ **Read this before counting CodeRabbit as an eye.** Its OSS free tier does not auto-review repositories under **10 stars**, and it reports that skip as a **green `success` check**. It reviewed nothing on **34 consecutive PRs (#36–#69)** while every one of them showed a passing CodeRabbit tick ([ERR-018](../docs/ledgers/errors.md)). Until the repo clears 10 stars, an automated review happens **only if someone comments `@coderabbitai review` on the PR** — which does work, but is subject to a rolling account-level rate limit that can refuse a PR outright. **All three outcomes return a green `success`:** `Review completed`, `Review skipped: manual review required for this OSS repository`, and `Review rate limited`. The colour tells you nothing; only the `description` does.
