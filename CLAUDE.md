...
- **Tests:** <!--fact:tests_unit-->558<!--/fact--> unit + <!--fact:tests_integration-->68<!--/fact--> integration (<!--fact:tests-->626<!--/fact--> collected; integration needs Docker or `$JOBFETCHER_DB_URL`), ~95% coverage in CI, `ruff` clean.
...
- **Build workflow ([ADR-0013](docs/adr/0013-enforcement-gate-trio-branch-pr.md)):** each build unit runs the **gate trio** — `/start-step` (entry) → implement → `/review-step` (code) → `/close-step` (exit) — with **two human checkpoints** (spec approved *before* code; approval *before* merge/tag). v0 *code* builds on a branch → PR → tag. **`main` is PR-only — for EVERYTHING, docs included.** That is not a convention any more: branch protection enforces it with `enforce_admins: true`, so a direct push is rejected outright ([ERR-015](docs/ledgers/errors.md)). The gate trio runs as an **agentic per-unit pipeline** — builder → review/simplify → **independent adversarial verifier (fresh context)** → scribe/close → security/verify — fanned out across *independent* units; **the human is the additional independent eye per PR — and from 2026-07-21 until 2026-09-05 the only automated one was the Examiner** ([ADR-0019](docs/adr/0019-agentic-build-orchestration.md)). CodeRabbit's OSS free tier stopped auto-reviewing repos under 10 stars and reports the skip as a **green check**, so 34 PRs merged believing in a reviewer that did not run ([ERR-018](docs/ledgers/errors.md)). **The threshold was cleared 2026-09-05 (11 stars) and auto-review is back** — but `Review completed`, `Review skipped` and `Review rate limited` are all a green `success`, so **read the check's description, not its colour**. **The canonical, invocable squad procedure** — the current roles (Investigator → Surgeon → single Examiner), the **severity-gated auto-merge** policy (crucial → human, non-crucial → auto-pilot), and how to run it — lives in [`.agents/agentic-workflow.md`](.agents/agentic-workflow.md); say *"run the agentic workflow for X"* to invoke it.
