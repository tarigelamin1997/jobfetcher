...
**v0 runs this entirely in the Lambda — no AWS service beyond Aurora, and no Bedrock.** `language-detect` uses the **`lingua`** library (English-vs-Arabic ≈ 99.9% — disjoint scripts; verified by a behavioral gate on real probe JDs, low-confidence → **flag, not drop**; replayable over immutable bronze), and **`embed` is deferred to M2** — it powers pgvector dedup *blocking*, which v0 doesn't need (v0 dedup is **exact-id only**). So **v0 silver is quota-independent**: only scoring (Step 5) needs Bedrock. The expensive *semantic* work — skills, sections, the score — is the **LLM's** job at scoring ([ADR-0012](adr/0012-model-agnostic-llm.md)), not silver's.
