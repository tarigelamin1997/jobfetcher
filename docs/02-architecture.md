...
- **Gold — filter to candidates.** The **LLM `FilterStrategy`** ([ADR-0016](adr/0016-llm-dissection-at-silver.md)) selects the "likely-fit" subset from the *already-dissected* silver fields (normalized title / seniority / skills / location vs the profile). Below-bar rows stay in bronze/silver for the market-wide analytics.
