...
Implement the **JSearch adapter** behind the source-port interface: call per query (keywords + `country` + `date_posted`), **paginate to a config page-cap**, with rate-limit/backoff + **quota-header awareness** (stop gracefully near quota, landing what you have). **Bronze:** write each raw result *untouched* to S3 `raw/jsearch/{date}/` + a `bronze_posting` row (immutable). **Silver:** normalize through the Pydantic contract into `posting`; clean → **`lingua` language-detect** (English filter; behavioral gate on real probe JDs, low-confidence → flag not drop) → fingerprint → field-map; **exact-id dedup on re-fetch** (no embeddings/Bedrock in v0 — the embed step + pgvector blocking are M2). **This whole ingestion→silver→gold half is Bedrock-independent — buildable + runnable now; only Step 5 (scoring) waits on the [ERR-001](ledgers/errors.md) quota.** Thread the **correlation `run_id`**. Request-budget knobs (queries, page-cap, date window) live in config.
