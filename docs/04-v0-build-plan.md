...
Modules/resources: S3 bucket (`force_destroy = true`), Secrets Manager entries (empty placeholders), **Aurora Serverless v2 + Data API** ([ADR-0014](adr/0014-operational-store-aurora-serverless-data-api.md); **no VPC**), the Lambda + IAM role (least-privilege: only its S3 prefix, its secrets, Bedrock invoke, SES send, **RDS Data API** access), EventBridge daily rule, SES identity. Populate secrets once via CLI (never committed).
