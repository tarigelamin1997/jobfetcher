...
| [INV-003](INV-003-silent-fetch-stop/) | Silent fetch stop — a run that fetches nothing reports success | `fixed` | non-crucial² | A 429 from JSearch hits a **bare `return`** (`jsearch_source.py:190`) — no log, no counter, no signal — four lines below a comment demanding a broken credential "FAIL LOUDLY". 3+ days of `200` + `fetched: 0`, no `raw/` since 2026-09-01, all 3 alarms `OK` → **legibility + cadence fix live-proven 2026-09-22** (119 fetched in the first clean quota cycle) | [ERR-017](../ledgers/errors.md) · [B-12](../ledgers/backlog.md) (rung 3 built as a digest alert; open until live-proven) |
| [INV-004](INV-004-alarm-escalation/) | The alarms work — nothing escalates | `handoff-ready` | non-crucial³ | The `returned-500` alarm fired **29 times** during the 38-day outage and was ignored 29 times; the dead-man has **never** fired in its life. CloudWatch cannot express "failing 3 days running" (24h evaluation ceiling), so escalation must come from the pipeline reading `run_log`. **A response problem, not a detection problem** | [B-5](../ledgers/backlog.md) · blocks the [B-12](../ledgers/backlog.md) alarm route only — B-12's digest alert is deployed (2026-09-23) |
| [INV-005](INV-005-silent-llm-credit-exhaustion/) | Silent LLM credit exhaustion: an empty DeepSeek account reports success | `in-progress` | crucial⁴ | A 402 is typed, counted as `billing_blocked` and logged once, and then **nothing reads it**. The run returns `200` and the digest still sends, reading "no new matches". It already happened for **9 days** (2026-08-25 → 09-02, 1,672 blocked calls), hidden behind ERR-010. Balance $0.54 (≈34 postings) on 2026-09-23. → **both rungs built + Examiner clean pass 2026-09-23**: a blocked run's digest leads with an out-of-credit banner, and one best-effort `GET /user/balance` per run warns under $2. **Merged (#80) and deployed 2026-09-23 18:09 UTC**; awaiting a delivered digest showing the banner (low-credit expected 09-24, out-of-credit 09-25) | [ERR-019](../ledgers/errors.md) · [B-10](../ledgers/backlog.md) (partly instrumented) · [B-12](../ledgers/backlog.md) (banner reused) · [ERR-010](../ledgers/errors.md) · follow-ups [B-16/17/18](../ledgers/backlog.md) |

¹ crucial for the recommended fix (a public capture endpoint — live infra + auth); a rung-1 interim is non-crucial.
² non-crucial for rungs 1–2 (`src/`-only observability); **rung 3 adds a CloudWatch alarm → crucial** (live infra, both checkpoints).
³ non-crucial for rung 1 (`src/`-only — reuses the existing `get_last_digest_sent_at` port method); **rungs 2–3 touch alarms + notification routing → crucial**.
⁴ crucial because rung 2 adds a new external call (DeepSeek `GET /user/balance`) to the unattended daily run. Rung 1 alone (reading the existing counter, `src/`-only) would be non-crucial. The approved scope is both rungs.
