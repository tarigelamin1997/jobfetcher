...
| [INV-003](INV-003-silent-fetch-stop/) | Silent fetch stop — a run that fetches nothing reports success | `handoff-ready` | non-crucial² | A 429 from JSearch hits a **bare `return`** (`jsearch_source.py:190`) — no log, no counter, no signal — four lines below a comment demanding a broken credential "FAIL LOUDLY". 3+ days of `200` + `fetched: 0`, no `raw/` since 2026-09-01, all 3 alarms `OK` | [B-12](../ledgers/backlog.md) · [B-5 adjacent](../ledgers/backlog.md) |
