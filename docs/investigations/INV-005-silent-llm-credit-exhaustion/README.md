...
**Status:** `in-progress` (built + Examiner clean pass 2026-09-23; awaiting Tarig's merge approval, the 2026-09-24 deploy, and a delivered digest showing the banner) · **Severity:** `crucial` (rung 2: a new external call) · **Owner of the fix:** a Surgeon, from the brief Tarig approved 2026-09-23 (see Resolution below)
...
1. **2026-09-24 (baseline, fix not deployed).** Not a fetch day, with nothing pending (09-23 scored all 35). Expect the **B-12** `partial_errors` banner, which is B-12's own proof and unrelated to this dossier. Record the balance read-only; it should still be $0.54.
2. **2026-09-25 (the BEFORE repro; fix almost certainly not deployed yet).** Expect `statusCode 200`, `score.billing_blocked > 0` (maybe `ingest.billing_blocked` too), one `LLM ACCOUNT OUT OF CREDIT` ERROR line, and a digest **sent with no credit banner**. If the run also deferred, expect no digest that day. Save `runs/2026-09-25/*.json`, the log line, and **a balance read ~30 min after the run**, which settles what `is_available` and `total_balance` read at exhaustion (rule (b) handles either). If credit is *not* exhausted, record the balance. It will be well under $2, and the repro moves to 09-28.
3. **First run after deploy, still unfunded (the AFTER proof).** Any day works, because unscored gold is retried every run (Evidence 5). Expect `llm_balance_usd` ≈ 0 (or negative), `billing_blocked > 0`, and a **delivered** digest whose subject begins `⚠ The LLM account is out of credit…` with the banner on top. **The proof is the email in the inbox, not the summary.** Also confirm that post-deploy `{"mode":"smoke"}` returns 200 and logs no balance call.
...
> 🚧 **Built and reviewed, not yet merged.** Examiner clean pass after one fix round. **Awaiting:** Tarig's merge approval (crucial tier), the deploy (a human checkpoint, planned 2026-09-24, before the 09-25 fetch day), and the live AFTER proof (Live proof step 3: a delivered digest that leads with the banner). The status moves to `fixed` only on that email, not on the merge.
