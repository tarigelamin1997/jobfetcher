...
## Decision
**Aurora PostgreSQL Serverless v2 + the RDS Data API**, with the **Lambda outside any VPC**. The Lambda calls the database over the **HTTPS Data API** (no Postgres wire protocol, no VPC), so it has direct, plumbing-free access to S3, Secrets Manager, Bedrock, and the public JSearch API. **`pgvector`** runs in the same Aurora DB for the M2 dedup-blocking embeddings (no separate vector store). *(Confirm at build: Data API + `pgvector` + scale-to-low/0 ACU on the chosen Aurora engine version.)*
...
- **Harder:** a higher per-ACU floor than a `t4g.micro` (mitigated by scale-to-low/0); the Data API has request/result-size limits (irrelevant at 10–30 rows/day); a hard dependency on Data API + `pgvector` on the chosen Aurora version (a build-time check).
