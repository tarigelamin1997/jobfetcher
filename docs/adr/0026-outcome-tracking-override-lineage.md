...
Record outcomes as an **append-only `application_event` log** written by a tiny CLI, and wire `score_override` as a **dual-write into the existing `score_event` lineage** — no pipeline change, no UI. Ships as migration **`0005_application_event`** (chains to `0004_score_event_lineage`); unreleased, rides **v0.7.0** alongside ADR-0025.
