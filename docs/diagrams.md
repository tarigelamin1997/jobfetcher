...
flowchart TB
  ...
  subgraph OP["Operational plane — AWS serverless"]
    ...
    subgraph OBS["Observability (v0.9)"]
      AL["3 CloudWatch alarms<br/>dead-man · Lambda Errors · returned 500"]
      SNS["SNS topic"]
      AL --> SNS
    end
    EB --> AL
  ...
  SI & SC -->|"LLM calls (OpenAI-compatible)"| DS
  ORC -.->|"GET /user/balance, once per run after scoring<br/>feeds the digest's credit banner (INV-005)"| DS
  SI <--> PG
  SC <--> PG
