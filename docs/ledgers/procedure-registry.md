...
| Data-contract / source normalization | ✅ Written → v0 | `SearchSpec` ([core/search_spec.py](../../src/jobfetcher/core/search_spec.py)) · `SourceAdapter` port + `JSearchSourceAdapter` (Step 4) · the **dissection contract** `DissectedPosting`/`Skill` (`core/models.py`, C-1); posting/score contracts at C-2 |
