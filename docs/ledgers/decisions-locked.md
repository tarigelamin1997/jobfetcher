...
| **LLM is model-agnostic** — config-selected Bedrock model via **Converse** (model id per task); **chosen model = `moonshot.kimi-k2-thinking`** (Kimi K2 Thinking), no longer waiting on Anthropic — but still gated by the account-wide quota until it lifts | Switching models is config, not code; selecting Kimi settles the *model*, not the *quota* (ERR-001) | [ADR-0012] · [ERR-001] |
...
| **Silver = LLM `Dissector` on every posting** (cheap model → structured `skills[]`/levels/sector/title/seniority/location/language) — populates the market-wide dimensional tables; **language is a byproduct field (no `lingua`)** | Skill-demand/sector analytics need *all* postings, not just gold; only **bronze** is live-runnable until ERR-001 lifts (rest test-able mocked) | [ADR-0016] |
