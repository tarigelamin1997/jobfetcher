...
**Logged:** 2026-09-04, from a live-stack review during the [ERR-016](errors.md) documentation audit — **not** reported by any alarm. **Status:** **partially closed 2026-09-04** — the blindness and the capacity mismatch are fixed ([ERR-017](errors.md), PRs #63/#64, config live), and **live-proven 2026-09-22**: 119 fetched in the first clean quota cycle (`runs/2026-09-22/19c08edf.json`). That sweep ended `partial_errors`, the first real early stop the alert below exists to announce. **Announcement built 2026-09-14, **deployed 2026-09-23 10:23 UTC** (build `9JcEtlkm…`, smoke `200`):** the daily digest opens with a one-line intake alert — what failed, and where to look, including a sweep-day run that crashed — on every day until a sweep succeeds (`core/intake.py`; see the [CHANGELOG](../../CHANGELOG.md)). This stays open until a real early stop has produced the banner in a delivered digest. The first chance is the 2026-09-24 digest, which should carry the 09-22 `partial_errors` banner. Investigated: **[INV-003](../investigations/INV-003-silent-fetch-stop/README.md)** (`fixed`).

**What still isn't solved.** A `200` with `fetched: 0` is now *diagnosable* — `fetch_stopped` says `rate_limited` / `budget_exhausted` / `not_a_fetch_day` in the durable run summary — but until the B-12 digest alert (deployed 2026-09-23, awaiting live proof) it was still not *announced*. No alarm covers "green but empty", so it takes someone looking. Rung 3 of the dossier (a `PIPELINE_ALARM`-style marker reusing INV-002's metric-filter wiring, or saying it in the digest) is deferred because it is live infra and must be **proven by making it fire**, not asserted. Same shape as **B-5**. **2026-09-14 — built as the digest option, not the alarm.** Per [INV-004](../investigations/INV-004-alarm-escalation/README.md), a fourth alarm in an inbox that had already ignored 29 would make things worse; the digest is the surface that is read every day. The alarm route stays deferred.
...
## B-19 · The pre-commit hook is not installed, so the local secret scan never ran

**Logged:** 2026-09-23, found while shipping [INV-005](../investigations/INV-005-silent-llm-credit-exhaustion/README.md) (#80). **Status:** open, cheap. Not fixed here: this was a docs-only unit, and installing a hook is something each clone does for itself, not a repo change.

**What.** `.pre-commit-config.yaml` declares four commit hooks: gitleaks, `ruff --fix`, `sync_docs --write` and `check_docs`. [`tests/README.md`](../../tests/README.md) counts the gitleaks hook as half of VG7's enforcement. In the working clone none of them has ever run: `.git/hooks/` holds only the `*.sample` files git ships, so there is no `pre-commit` hook, and [`docs/getting-started.md`](../getting-started.md) calls `pre-commit install` *optional*. That is how #80's deliberately fake test key `sk-SENTINEL-abcd1234` got through the commit and was first caught by CI's `secret-scan` job (see the 2026-09-23 18:09 UTC deploy entry in the [CHANGELOG](../../CHANGELOG.md)).

**Why it matters.** Nothing leaked. CI is the enforced gate, and it caught the key. The cost is *where* the catch happens: a real key committed locally is caught only after it has been pushed, when it sits in the branch history on GitHub and has to be rotated rather than amended away. It is the same "unarmed gate" class as [ERR-015](errors.md) (a rule wired but not armed) and [B-11](#b-11--the-tree-is-84-files-from-ruff-format-clean-and-the-gate-that-claimed-to-check-it-never-ran) (a gate that claimed to check and never ran). The config file makes a local mirror look present when it is not.
