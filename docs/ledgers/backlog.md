...
**Logged:** 2026-09-04, from a live-stack review during the [ERR-016](errors.md) documentation audit — **not** reported by any alarm. **Status:** **partially closed 2026-09-04** — the blindness and the capacity mismatch are fixed ([ERR-017](errors.md), PRs #63/#64, config live). **Announcement built 2026-09-14:** the daily digest now opens with a one-line intake alert — what failed, and where to look — on every day until a sweep succeeds (`core/intake.py`; see the [CHANGELOG](../../CHANGELOG.md)). This stays open until it is deployed and a real early stop has produced the banner. Investigated: **[INV-003](../investigations/INV-003-silent-fetch-stop/README.md)** (`fixed`).
