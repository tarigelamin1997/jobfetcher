...
- **A *human* external PR reviewer as a hard gate** → our own CI + the fresh-context adversarial Examiner + **CodeRabbit** (**not** a standing per-PR automated reviewer — it silently stopped auto-reviewing at 0 stars and reported the skip green for 34 PRs, [ERR-018](ledgers/errors.md); it runs on an explicit `@coderabbitai review` comment) suffice — only the *human* external-reviewer hard gate is cut. (A human co-reviewer stays optional.)
