...
- **⚠️ The JSearch sweep runs every 3rd day, not daily — and that is deliberate.** The Lambda still fires daily at 06:00 UTC (the dead-man alarm watches a 24-hour window and cannot be widened), but the *source sweep* costs quota: the free tier is **200 requests/month** and one sweep is `3 titles × 5 countries × 1 page = 15`. Daily sweeping needed ~450/month — **2.25× over** — so the tool silently ingested nothing for roughly half of every month until 2026-09-04 (the often-quoted **2.7× / ~19 dead days** are the *pre-fix* figures, from the 6-country `3 × 6 × 1 = 18` sweep at ~540/month; they are correct in [02-architecture](docs/02-architecture.md) and [ERR-017](docs/ledgers/errors.md), and were wrong here — spliced onto the post-fix sweep size) ([ERR-017](docs/ledgers/errors.md)). A non-fetch day still scores, digests and reports; the run summary says `fetch_stopped: not_a_fetch_day`. **Raise `FETCH_EVERY_N_DAYS` and the RapidAPI plan together — exceeding the quota stops intake for the rest of the month.** It is recorded (`fetch_stopped: rate_limited`); since the B-12 intake alert was deployed (2026-09-23) it also leads the daily digest — from the first clean quota cycle (2026-09-22), since earlier 429s are silent by design ([B-12](docs/ledgers/backlog.md)). It pages nobody.
- **⚠️ An empty LLM account is announced in the digest, and the daily run asks DeepSeek for its balance to know it** ([INV-005](docs/investigations/INV-005-silent-llm-credit-exhaustion/README.md); built 2026-09-23, merged as #80 and deployed the same day, awaiting live proof). An empty prepaid account turns every LLM call into HTTP 402. That was counted and logged, but the run still returned `200` and the digest read "no new matches", which is how it went unnoticed for 9 days in August ([ERR-019](docs/ledgers/errors.md)). Now a blocked run's digest **leads with an out-of-credit banner**, and under **$2.00** it warns ahead of time. To forecast, the daily run makes **one outbound `GET /user/balance` to DeepSeek, carrying the API key**, after scoring. It is best-effort (5 s, no retry, never fails the run) and records only a short error code, never a response body. The reading lands in the run summary as `llm_balance_usd`. It pages nobody either.
...
| **AWS SDK** | boto3 |
| **Tests** | pytest — **<!--fact:tests-->857<!--/fact--> collected: <!--fact:tests_unit-->789<!--/fact--> unit + <!--fact:tests_integration-->68<!--/fact--> integration** (integration needs Docker or `$JOBFETCHER_DB_URL`; live-key tests skip without a key), live smoke, ~95% coverage in CI (85% floor) |
