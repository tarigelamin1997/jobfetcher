...
### Added — scorer integrity: subscores + shadow totals (rides v0.9.0)
...
- Verified: **346 unit + 42 integration green** (+5 live-key skips; full suite **388 passed, 95.16% coverage**, 85% floor), `ruff` clean; independent fresh-context adversarial Examiner: **CLEAN PASS, zero blocking** (S1 test hardening + N6 boundary pin applied in `33a8ef4`). CRUCIAL tier — the PR goes to Tarig.
...
- **Milestone `milestone/pre-agentic-workflow-2026-07-07`** — the complete-documentation baseline (v0.1.0→v0.6.0 shipped + docs/diagrams swept current, 257 tests green). The clean checkpoint before the build phase switched to the **agentic squad workflow** (Investigator → Surgeon → Examiner per bottleneck; v0.7.0 is its first shipped output).

*Next up (in flight): v0.8.0 "the digest tells the truth" — digest scoping + graduation badges + duplicate collapse (PR #26, first auto-pilot unit) — and v0.9.0 scorer integrity above (Squad Run 4, built + examined clean, CRUCIAL tier — PR to Tarig). Then ops hardening (the ERR-008 invoke wrapper, Run-5 candidate).*

## [v0.7.0] — 2026-07-08 — "the pipeline remembers": score lineage + outcome tracking
